...
export default class AccountController extends IController {
  ...
  */
  public addAccountAndLogin = async (accountName: string, privateKeyHash: string, wallet: MetrixWallet) => {
  ...
  */
  public saveToFile = (accountName: string, mnemonic: string) => {
    ...
    element.download = `metrimask_${accountName}_${timestamp}.bak`;
    element.click();
  ...
  */
  public onAccountLoggedIn = async (isSessionRestore = false) => {
     ...
     * reopening the popup, we don't need to send the SEND_INPAGE_METRIMASK_ACCOUNT_VALUES event to
     * the inpage because window.metrimask.account has not changed.
     */
    if (!isSessionRestore) {
  ...
  */
  private recoverFromPrivateKeyHash(privateKeyHash: string): MetrixWallet {
  ...
  private getPrivateKeyHash(wallet: MetrixWallet) {
  ...
  */
  private getWalletInfo = async (sendInpageUpdate = true) => {
    ...
    if (infoDidUpdate) {
      ...
      if (sendInpageUpdate) {
        this.main.inpageAccount.sendInpageAccountAllPorts(METRIMASK_ACCOUNT_CHANGE.BALANCE_CHANGE);
      }
  ...
  */
   ...
   * 2 - Whenever the maxMetrixSend is requested, because even if the balance does not
   * change, the available UTXOs can change(which causes a change in maxMetrixSend).
   * For instance when a user sends Metrix, but that transaction has not confirmed yet,
   * the balance will not change, but calcMaxMetrixSend is able to account for those
   * unconfirmed UTXOs and update maxMetrixSend accordingly.
   */
  private updateAndSendMaxMetrixAmountToPopup = async () => {
    if (!this.loggedInAccount || !this.loggedInAccount.wallet || !this.loggedInAccount.wallet.mjsWallet) {
